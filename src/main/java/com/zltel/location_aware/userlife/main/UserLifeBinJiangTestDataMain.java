...
import com.zltel.location_aware.userlife.map.UserLifeBinJiangTestDataMap;
import com.zltel.location_aware.userlife.reduce.UserLifeBinJiangTestDataReduce;
 ...
 *
 */
public class UserLifeBinJiangTestDataMain {
	private static Logger logout = LoggerFactory.getLogger(UserLifeBinJiangTestDataMain.class);
	 ...
	 * 过滤 IMSI 的文件
	 */
	public static final String FILTER_IMSI = BASE_PATH + "userlife/res/filter_imsi.txt";
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			if (fs.exists(new Path(FILTER_IMSI))) {
				logout.info("userlife res:" + FILTER_IMSI + "  exists");
			} else {
				String m = "userlife res:" + FILTER_IMSI + "  does not exists !";
				logout.error(m);
				throw new RuntimeException(m);
			}
			...
			job.setJarByClass(UserLifeBinJiangTestDataMain.class);
			// 设置Map/Reduce 处理类
			job.setMapperClass(UserLifeBinJiangTestDataMap.class);
			// 设置Map/Reduce 中间结果指
			...
			job.setReducerClass(UserLifeBinJiangTestDataReduce.class);
