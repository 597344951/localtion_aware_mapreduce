 ...
 * @see
 */
public class HfrlMap extends MapReduceBase implements
    ...
    try {
      ...
      if (tmp.length > 1) {
        ...
        if (colum.length >= 39) {
          ...
          up_bytes = colum[21];
          down_bytes = colum[22];
          fci = Integer.parseInt("".equals(colum[colum.length-2]) ? "0" : colum[colum.length-2], 16)
              + "";
          rat = colum[7];
          ...
          output.collect(new Text(rkey), new Text(rat + "," + imsi + ","
              + up_bytes + "," + down_bytes));
            ...
            if (stime != null && stime.length() > 0) {
              try {
                ...
                if ("0".equals(cdr_id) || "2".equals(cdr_id)
                    || "4".equals(cdr_id)) {
                  type = "2";
                  String rkey = etime + "_" + type + "_" + ci;
                  output.collect(new Text(rkey), new Text(imsi));
                } else if ("1".equals(cdr_id) || "3".equals(cdr_id)
                    || "5".equals(cdr_id)) {
                  type = "3";
                  String rkey = etime + "_" + type + "_" + ci;
                  output.collect(new Text(rkey), new Text(imsi));
