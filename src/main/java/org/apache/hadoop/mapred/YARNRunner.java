...
public class YARNRunner implements ClientProtocol {
	 ...
	 * 
	 * @param resMgrDelegate
	 *            the resource manager delegate to set to.
	 */
	...
	void addHistoryToken(Credentials ts) throws IOException, InterruptedException {
		...
		if (UserGroupInformation.isSecurityEnabled() && (hsProxy != null)) {
			...
			Text service = SecurityUtil.buildTokenService(((MRClientProtocol) resMgrDelegate).getConnectAddress());
	...
	public ApplicationSubmissionContext createApplicationSubmissionContext(Configuration jobConf, String jobSubmitDir,
		...
		URL yarnUrlForJobSubmitDir = ConverterUtils.getYarnUrlFromPath(defaultFileContext.getDefaultFileSystem()
		...
		MRApps.addLog4jSystemProperties(logLevel, logSize, numBackups, vargs, jobConf);
		...
		MRApps.setEnvFromInputString(environment, conf.get(MRJobConfig.MR_AM_ADMIN_USER_ENV));
		...
		ContainerLaunchContext amContainer = ContainerLaunchContext.newInstance(localResources, environment, vargsFinal,
				null, securityTokens, acls);
